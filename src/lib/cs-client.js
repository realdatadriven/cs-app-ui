...
export function createCSClient(initialConfig = {}) {
  ...
  async function apiCall(endpoint, payload = null, options = {}) {
    const { method = 'POST', timeoutMs = 45000, overrideToken } = options;
    ...
    try {
      ...
      clearTimeout(timeoutId);
      if (err.name === 'AbortError') return { success: false, msg: 'Request timeout' };
      return { success: false, msg: err.message || 'API call failed', error: err };
    }
  }
  ...
  const client = {
    ...
    async login(username, password) {
      ...
      const res = await apiCall('/dyn_api/login/login', { data: { username, password } });
